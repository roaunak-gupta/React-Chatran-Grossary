    ...
    return (
        <div className='relative mt-24'>
            ...
            <div className='absolute inset-0 flex flex-col items-center md:items-end md:justify-center pt-16 md:pt-0 md:pr-24'>
                <div>
                    <h1 className='text-2xl md-text-3xl font-semibold text-primary-darkgreen mb-6'>Why We Are The Best?</h1>
                    {features.map((features, index) => (
                        <div key={index} className='flex items-center gap-4 mt-2'>
                            <img src={features.icon} alt={features.title} className='md-w-11 w-9' />
                            <div>
                                <h3 className='text-primary-darkgreen text-lg md:text-xl font-semibold'>{features.title}</h3>
                                <p className='text-gray-500/70 text-sm md:text-sm'>{features.description}</p>
                            </div>
                        </div>
                    ))}
                </div>
